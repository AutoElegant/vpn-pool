"""Коллектор: качает источники → парсит → дедуп → проверяет → пишет пул.

Запускается на раннере GitHub Actions. Проверка доступности из РФ
сюда не входит: check-host не отвечает облачным адресам, поэтому
её делает сервер при синхронизации (см. core/sync.py).
"""
